...
Method{1}='LFS method';
Method{2}='Xiaos method';
Method{3}='Improved Ajallouians method';
Method{4}='Improved Zhangs method';
Method{5}='Improved EL method';
Method{6}='Improved IDMF method';
Method{7}='Improved gray autocorrelation method';
Method{8}='Improved pattern autocorrelation method';
...
addpath( '.\Code outside the main algorithm\');
...
if img_num > 0 
    ...
    for k = LoopBeg:LoopEnd %read picture one by one
       ...
       %%
        ...
        switch MethodInd
            case 1
                [NumCowarp,NumCoweft]=GetWaWeCNumNSMethod(warpSeg,weftSeg,estructpfea,isshowpic);
            case 2
                [NumCowarp,NumCoweft]=GetWaWeCycNum(warpSeg,weftSeg,estructpfea,isshowpic);
            case 3
                [NumCowarp,NumCoweft]=GetPRSizeFirstYRepeatMethod(warpSeg,weftSeg,espclare,Caresult);
            case 4
                [NumCowarp,NumCoweft]=GetSRepeatUnitMethod(warpSeg,weftSeg,espclare,Caresult,isshowpic);
            case 5
                [NumCowarp,NumCoweft]=GetPRSizeELMethod(warpSeg,weftSeg,espclare,Caresult);
            case 6
                [NumCowarp,NumCoweft]=GetPRSizeIDMFMethod(im,warpSeg,weftSeg,isshowpic);
            case 7
                [NumCowarp,NumCoweft]=GetPRSizeAutocorrelationMethod(im,warpSeg,weftSeg,isshowpic);
            case 8
                [NumCowarp,NumCoweft]=GetPRSizePatternAutocoMethod(warpSeg,weftSeg,espclare,Caresult,isshowpic);
        end
        toc
        Timtemp(k)=toc;
       ...
       %%
        if isshowpic==1
            ...
            PRImwrite_path=strcat(imwrite_path,'Repeat Units Drawing in Original Image\');
            SaveImPatRepResult(Orim,warpSeg,weftSeg,NumCowarp,NumCoweft,PRImwrite_path,image_name,imformat);
            RPPRImwrite_path=strcat(imwrite_path,'Repeat Units Drawing in Pre-recognized Weave Pattern\');
            SaveRPPatRepResult(im,warpSeg,weftSeg,espclare,Caresult,NumCowarp,NumCoweft,RPPRImwrite_path,image_name,imformat);
            RPPRImwrite_path=strcat(imwrite_path,'Repeat Units Drawing in Corrected Weave Pattern\');
            SaveRPPatRepResult(im,warpSeg,weftSeg,Finalespclare,Caresult,NumCowarp,NumCoweft,RPPRImwrite_path,image_name,imformat);
        end
    end
    if isshowpic==0
        fclose(fid);
